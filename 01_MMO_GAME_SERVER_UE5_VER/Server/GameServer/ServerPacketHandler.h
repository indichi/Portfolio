...
public:
	static void Init()
	{
		...
		GPacketHandler[PKT_C_TRY_LOGIN] = [](PacketSessionRef& session, BYTE* buffer, int32 len) { return HandlePacket<Protocol::C_TRY_LOGIN>(Handle_C_TRY_LOGIN, session, buffer, len); };
	...
	static SendBufferRef MakeSendBuffer(Protocol::S_TRY_LOGIN& pkt) { return MakeSendBuffer(pkt, PKT_S_TRY_LOGIN); }
